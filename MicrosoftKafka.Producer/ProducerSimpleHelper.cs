...
namespace MicrosoftKafka.Producer
{
    ...
    /// </summary>
    public class ProduceSimpleHelper
    {
        ...
        private static int totalPartitionCount = 5;
        ...
        private static void ProduceByPartitionerClass(ProduceSimpleHelperOption produceOptions, int partitionCountInZookeeper)
        {
            using (KafkaSimpleManager<byte[], Message> kafkaSimpleManager = new KafkaSimpleManager<byte[], Message>(kafkaSimpleManagerConfig))
            {
                ...
                stopwatch2.Start();
                int nProducers = kafkaSimpleManager.InitializeProducerPoolForTopic(1, ClientID, ProducerRequestId++, produceOptions.Topic, true, producerConfigTemplate, false);
                stopwatch2.Stop();
                ...
                while (true)
                {
                    ...
                    catch (Exception e)
                    {
                        ...
                        TopicMetadata topicMetadata = kafkaSimpleManager.RefreshMetadata(1, ClientID, ProducerRequestId++, produceOptions.Topic, true);
                ...
                while (true)
                {
                    ...
                    try
                    {
                        ...
                        if (DateTime.UtcNow > lastTimeRefreshMetadata.AddMinutes(10))
                        {
                            ...
                            {
                                nProducers = kafkaSimpleManager.InitializeProducerPoolForTopic(1, ClientID, ProducerRequestId++, produceOptions.Topic, true, producerConfigTemplate, false);
                                totalPartitionCount = kafkaSimpleManager.GetTopicMetadta(produceOptions.Topic).PartitionsMetadata.Count();
                                targetPartitionID = produceOptions.PartitionId >= 0 ? produceOptions.PartitionId : rand.Next(totalPartitionCount);
                                Logger.InfoFormat("Will try get producer for partition:{0} ", targetPartitionID);
                                producer = kafkaSimpleManager.GetProducerOfPartition(produceOptions.Topic, targetPartitionID, false);
                    ...
                    catch (Exception e)
                    {
                        ...
                        try
                        {
                            producer = kafkaSimpleManager.RefreshMetadataAndRecreateProducerOfOnePartition(1, ClientID, ProducerRequestId++, produceOptions.Topic, targetPartitionID, true, true, producerConfigTemplate, false);
                        }
                        catch (Exception ex)
                        {
